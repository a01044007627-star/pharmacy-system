...
type ReportData = {
  summary: { total_purchases: number; purchases_count: number; average_purchase: number; paid_amount: number; due_amount: number }
  daily: DailyRow[]
  error?: string
}

function exportPurchasesReportCsv(daily: DailyRow[], summary: ReportData["summary"]) {
  const header = ["التاريخ", "عدد الفواتير", "الإجمالي"]
  const rows = daily.map((d) => [d.date, String(d.count), String(d.total)])
  const footer = ["الإجمالي", String(summary.purchases_count), String(summary.total_purchases)]
  saveCsv("تقرير_المشتريات.csv", [header, ...rows, footer])
}
...
export function PurchasesReportView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/purchases?${params.toString()}`, { cache: "no-store" })
  ...
  const cards = useMemo(() => {
    if (!summary) return []
    return [
      { label: "إجمالي المشتريات", value: money(summary.total_purchases), tone: "text-brand" },
      { label: "عدد الفواتير", value: summary.purchases_count.toLocaleString("ar-EG"), tone: "text-slate-950" },
      { label: "متوسط الفاتورة", value: money(summary.average_purchase), tone: "text-blue-700" },
  ...
  return (
    <PageAccess permission="reports:read" message="ليست لديك صلاحية عرض تقارير المشتريات">
      <section dir="rtl" className="page-container space-y-4 py-4 text-right sm:py-6">
        <DashboardPageHeader
          title="تقرير المشتريات"
          subtitle="تحليل يومي لحركة المشتريات والمدفوعات."
          icon={BarChart3}
          actions={(
            <>
              ...
              <Button variant="outline" className="h-10 rounded-xl" onClick={() => exportPurchasesReportCsv(daily, summary!)} disabled={!data}>
        ...
        <Card className="overflow-hidden rounded-3xl border-slate-200 shadow-sm">
          {loading ? (
            ...
            <EmptyState icon={Calendar} title="لا توجد بيانات" description="لم يتم العثور على مشتريات في النطاق المحدد." />
          ) : (
