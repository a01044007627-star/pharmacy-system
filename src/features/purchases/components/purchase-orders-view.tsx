...
export function PurchaseOrdersView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "فشل تحميل أوامر الشراء")
    } finally {
      setLoading(false)
    }
  ...
  async function updateStatus(order: OrderRow, newStatus: string) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "فشل تحديث حالة الأمر")
    }
  }
      ...
      <section dir="rtl" className="page-container space-y-4 py-4 text-right sm:py-6">
        ...
        <Card className="rounded-3xl border-slate-200 shadow-sm">
          <CardContent className="grid gap-3 p-4 md:grid-cols-2">
            <div className="relative">
              ...
              <Input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1) }} placeholder="رقم الأمر أو اسم المورد..." className="h-11 rounded-2xl pr-10 font-bold" />
            </div>
        ...
        <Card className="overflow-hidden rounded-3xl border-slate-200 shadow-sm">
          {loading ? <SkeletonRows count={6} /> : rows.length === 0 ? (
            <EmptyState icon={ClipboardList} title="لا توجد أوامر شراء" description="لم يتم إنشاء أي أمر شراء بعد." />
          ) : (
              ...
              <TableBody>
                {rows.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell className="font-bold">{order.supplier_name}</TableCell>
                    <TableCell>{order.branch?.name ?? "—"}</TableCell>
                    <TableCell className="text-center font-black">{money(order.total)}</TableCell>
                    <TableCell className="text-center font-black text-emerald-700">{money(order.paid_amount)}</TableCell>
                    <TableCell className="text-center font-black text-rose-600">{money(order.due_amount)}</TableCell>
