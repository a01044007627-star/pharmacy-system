...
// ──────────────────────────────────────────────────────────────────────────────
// Pure Code128-B barcode renderer — zero external dependencies
// ──────────────────────────────────────────────────────────────────────────────
...
function encodeCode128(text: string): string {
  ...
  checksum %= 103
  const bars =
    CODE128_TABLE[START_B] +
    chars.map((c) => CODE128_TABLE[c] ?? "").join("") +
    CODE128_TABLE[checksum] +
    CODE128_TABLE[STOP] +
    "11"
  return bars
}
...
function BarcodeSVG({ value, format = "CODE128" }: { value: string; format?: string }) {
  ...
  return <svg ref={svgRef} className="mx-auto max-h-[40px] max-w-full" />
}
