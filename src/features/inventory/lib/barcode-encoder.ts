/**
 * Code128-B barcode encoder — pure utility, zero dependencies.
 * Can be imported in both server and client components.
 *
 * Standard reference: ISO/IEC 15417:2007 (Code 128 bar code symbology)
 */

// ── Lookup table: Code128 character index → bar/space pattern (1=bar, 0=space) ──
// Each value encodes 11 modules. Indices 0-102 are the printable symbols;
// 103 = START B, 106 = STOP.
...
/**
 * Encodes an ASCII string into a Code-128B bit pattern string.
 * Returns a string of "1" (bar) and "0" (space) characters.
 *
 * Characters outside printable ASCII (32–126) are silently skipped.
 */
export function encodeCode128(text: string): string {
  // Map each character to its Code128-B index (charCode - 32)
  const indices: number[] = []
  for (const ch of text) {
    const idx = ch.charCodeAt(0) - 32
    if (idx >= 0 && idx <= 94) indices.push(idx)
  }

  // Weighted checksum: START_B + Σ(value × position) mod 103
  let checksum = START_B
  indices.forEach((v, i) => (checksum += v * (i + 1)))
  checksum %= 103

  return (
    CODE128[START_B] +
    indices.map((v) => CODE128[v] ?? "").join("") +
    CODE128[checksum] +
    CODE128[STOP] +
    "11" // termination bar
  )
