...
import { DollarSign, RefreshCw, Search, Trash2 } from "lucide-react"
...
export function PriceOffersView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/sales/price-offers?${params.toString()}`, { cache: "no-store" })
      const data = await response.json().catch(() => ({})) as ResponseData
      if (!response.ok) throw new Error(data.error ?? "فشل تحميل عروض الأسعار")
      setRows(data.offers ?? [])
      setTotalPages(data.pagination?.totalPages ?? 1)
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "فشل تحميل عروض الأسعار")
    } finally {
      setLoading(false)
    }
  }, [auth.activeBranchId, auth.activePharmacyId, page, query])
  ...
  async function deleteOffer(offer: OfferRow) {
    if (!window.confirm(`حذف عرض السعر "${offer.title}"؟`)) return
    try {
      const params = new URLSearchParams({ id: offer.id, pharmacy_id: auth.activePharmacyId! })
      const response = await fetch(`/api/sales/price-offers?${params.toString()}`, { method: "DELETE" })
      const data = await response.json().catch(() => ({})) as { error?: string }
      if (!response.ok) throw new Error(data.error ?? "فشل حذف عرض السعر")
      toast.success("تم حذف عرض السعر")
      await load()
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "فشل حذف عرض السعر")
    }
  }
      ...
      <section dir="rtl" className="page-container space-y-4 py-4 text-right sm:py-6">
        ...
        <Card className="overflow-hidden rounded-3xl border-slate-200 shadow-sm">
          {loading ? <SkeletonRows count={5} /> : rows.length === 0 ? (
            <EmptyState icon={DollarSign} title="لا توجد عروض أسعار" description="لم يتم إنشاء أي عرض سعر بعد." />
          ) : (
              ...
              <TableBody>
                {rows.map((offer) => (
                  <TableRow key={offer.id}>
                    <TableCell className="font-black text-slate-950">{offer.title}</TableCell>
                    <TableCell>{offer.branch?.name ?? "—"}</TableCell>
                    <TableCell className="text-center">
                      <Badge variant="outline" className={cn("font-black", statusColor(offer.status))}>{statusLabel(offer.status)}</Badge>
                    </TableCell>
                    <TableCell className="text-center text-xs font-bold">{new Date(offer.created_at).toLocaleString("ar-EG")}</TableCell>
                    <TableCell>
                      <div className="flex items-center justify-center gap-1">
                        {(auth.isDeveloper || auth.can("sales:write")) ? (
                          <Button size="icon" variant="ghost" className="text-rose-600 hover:bg-rose-50" onClick={() => void deleteOffer(offer)} title="حذف">
